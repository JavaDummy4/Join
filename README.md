Any use, modification, reproduction, or distribution of this software is prohibited without the author’s prior written consent.
...
Join is a self-contained object-relationship modeling tool — think of it as a lightweight, personal knowledge-graph editor that lives entirely in a single .html file. No install, no server, no account, no build step. Open it in a browser and start connecting things.

A city council and its 180 employees. A photo of a table and everything sitting on it. A 1930 pulp sci-fi story and how its characters relate. A car brand and the country it's built in. Join doesn't care what the subject is — it only cares about objects and how they connect.

Why this exists
Most relationship-modeling tools are either too generic (a whiteboard app that doesn't know what a "class" is) or too narrow (a genealogy tool that only understands people). Join sits in between: it ships with 80+ predefined object classes across dozens of real-world domains — each with its own icon, color, and relevant fields — and it's trivial to add a new one. Every object also gets a universal Remark field, so nothing you learn about it has nowhere to go.

And because the whole thing is one plain HTML file with a documented JSON format underneath, any AI can generate a new, ready-to-load model for you — just point it at Modelfile-Format-Spezifikation.md.
...
80+ built-in classes — municipal administration, tourism, aviation alliances, the automotive industry, camera equipment, name etymology, literature, household items, freshwater/saltwater fish, whales, and more — each with its own fields, icon, and color
...
📷 Photo → instant model — point your camera at a scene (say, a table with a few things on it) and Claude Vision identifies the room, the surface, and every object on it, wiring up the relationships automatically (Room ← Surface ← Objects). Requires your own Anthropic API key — see below.
Combine and share without losing data

Merge two models and objects with the same name and class are automatically unified into one — filled-in fields are combined, and if two models disagree on a value, the conflict is recorded in Remark instead of silently overwritten
...
The class system
Every object belongs to exactly one class, and every class belongs to a group (e.g. "Tourism & Gastronomy," "Aquatic Animals," "Household"). A class defines an icon, a color, and up to a handful of fields relevant to that kind of thing — a Whale cares about species and protection status; a Kitchen Appliance cares about power and material. Objects that don't fit anywhere yet can stay Unassigned until you're ready.
...
Photo analysis calls the Anthropic API directly from your browser (bring-your-own-key). A Claude.ai subscription does not cover this — the chat subscription and API usage are billed separately. To use it:
...
Cost is small: with Claude Haiku 4.5 (the model this feature uses), a single photo typically costs a fraction of a cent. We'd still recommend setting a low monthly spending limit in the console, just in case.

Model files
A model is just JSON: a list of objects (nodes) and a list of relationships (edges) between them. The exact schema — including the full, current list of every class and its fields — is documented in Modelfile-Format-Spezifikation.md, written specifically so an AI (or a careful human) can generate a new, valid model file without ever having seen the tool run.
